...
def getMyData(message, api_key, bot):
    """
    This function retrieves user data using the CodeGPT API and formats it into a readable string.
    It then sends the formatted response back to the user via a bot.
    """

    try:
        # Initialize the CodeGPT API with the provided API key
        codegpt = CodeGPT(api_key)
        
        # Retrieve the user data from the CodeGPT API
        user_data = codegpt.me()

        # Format the user_data object into a string
        formatted_data = json.dumps(user_data, indent=4)

        # Send the formatted response back to the user via the bot
        bot.reply_to(message, formatted_data)
    
    except Exception as e:
        # Handle any exceptions that occur and send an error message to the user
        error_message = f"An error occurred while retrieving your data: {str(e)}"
